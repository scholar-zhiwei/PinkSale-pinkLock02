...
contract PinkLock02 is IPinkLock, Pausable {
    ...
    function vestingLock(
        ...
        //10000 将100扩大成100 防止小数出现，前端在讲其缩小到100
    ...
    function _lockLpToken(
        ...
        //tokenInfo.token == address(0)这个token还未锁过
    ...
    function _vestingUnlock(Lock storage userLock) internal {
        ...
        //第一次释放时userLock.unlockedAmount为0
        ...
        //tokenInfo.amount为用户存该token的总数量
        ...
        //减去withdrawable后tokenInfo.amount为0 说明该token就剩下最后一个交易锁了
        if (tokenInfo.amount == 0) {
            if (isLpToken) {
                _lpLockedTokens.remove(userLock.token);
            } else {
    ...
    function _safeTransferFromEnsureExactAmount(
        ...
        //require(IERC20(token).balanceOf(sender) >= amount);
